...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            for i, data in tqdm(enumerate(val_loader), desc='Evaluating samples', total=len(val_loader)):
                y, x, mean, std = data
                y = y.cuda()
                x = x.cuda()
                mean = mean.cuda()
                ...
                gens = torch.zeros(size=(y.size(0), cfg.num_z_test, cfg.im_size, cfg.im_size, 2)).cuda() #Is this a 2 for re im or a 2 for input channels i.e. now a 4
                ...
                for j in range(y.size(0)):
                    reconstruction = ndimage.rotate(torch.tensor(tensor_to_complex_np((torch_reconstruction[j] * kappa_std + kappa_mean).cpu())).numpy(), 180)
                    truth = ndimage.rotate(torch.tensor(tensor_to_complex_np((torch_truth[j] * kappa_std + kappa_mean).cpu())).numpy(), 180)
